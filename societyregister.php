...
<body><!-- header -->
	...
	<div id="main">
		<h1 id="title">Society Register </h1>
	<?php 
		require "core.php";
	require "connect.php";
	...
	if(!$link||!@mysqli_select_db($link,'webhack_login'))
		...
		if( isset($_POST['p']) && isset($_POST['cp'])&& isset($_POST['i']) && isset($_POST['s'])){
			if(!empty($_POST['i']) && !empty($_POST['p']) &&!empty($_POST['s']) &&!empty($_POST['cp'])){
				...
				if($password!=$con_password) echo 'password do not match';
					...
					$query="INSERT INTO `society info`(institute_name,society_name,pass) VALUES ( '$institute','$society','$password')";
					echo 'society registered';
	 ...
	 ?>
		...
		<form id="form" method="POST" action="societyregister.php">

			<input required="required" class="form-control" type="text" name="i" placeholder="institute"><br>
			<input required="required" class="form-control" type="text" name="s" placeholder="society"><br>
			...
			<button id="submit" class="btn btn-default">Register society</button>
		</form>
