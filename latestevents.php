...
<?php 
	...
	function findsoc($data){
	 	global $id;
	 global $link;
	$query="SELECT `$data` FROM `society info` ";
	$result=mysqli_query($link,$query);
	$output=mysqli_fetch_assoc($result);
	return $output["$data"];
}
...
<body>
 ...
 <thead><th>Event Name</th><th>Institute Name</th><th>Society Name</th><th>Date</th><th>View</th></thead>
 <tbody>
 	<?php
 		$sql = "SELECT id,event, institute_name, society_name,date FROM event";
...
if(mysqli_affected_rows($link)>0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<tr>" ."<td>". $row["event"] ."</td>"."<td>". $row["institute_name"]."</td>"."<td>". $row["society_name"]."</td>"."<td>". $row["date"]."</td><td>";

        echo '<form method="POST" action="single.php"><input type="hidden" name="id" value="'.$eid.'"><button type="submit">View</button></form></td></tr>';
    }}
 	?>
